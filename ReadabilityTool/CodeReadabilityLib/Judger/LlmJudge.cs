...
namespace CodeReadabilityLib.Judger
{
    ...
    public abstract class LlmJudge : IReadabilityJudge
    {
        ...
        public async Task<IReadOnlyList<ReadabilityScore>> EvaluateAsync(ReadabilityEvaluationRequest request, CancellationToken ctoken = default)
        {
            ...
            List<ReadabilityScore> scores = new List<ReadabilityScore>(parsed?.Scores?.Count ?? 0);
            if (parsed?.Scores != null)
            {
                foreach (JudgeScoreItem item in parsed.Scores)
                {
                    string name = (item.Id ?? string.Empty).Trim();
                    if(name.Length == 0)
                    {
                        continue;
                    }
            ...
            //foreach (MetricDefinition metric in request.Metrics)
            //{
            //    if (!present.Contains(metric.Id))
            //    {
            //        scores.Add(new ReadabilityScore(metric.Id, 0));
            //    }
            //}
        ...
        protected virtual string BuildSystemPromt()
        {
            return "You are a strict, deterministic code readability evaluator. " +
                   ...
                   "{\"scores\":[{\"id\":string,\"score\":int}]} " +
                   "Rules: 'score' must be an integer. " +
                   "It should be between the minimum and maximum values for each metric if said values are present." +
                   "Use the metrics's definition to determine how assess the metric." +
        ...
        protected virtual string BuildUserPrompt(ReadabilityEvaluationRequest request)
        {
            ...
            {
                sb.AppendLine("Context: " + request.Context!.Trim());
            }

            sb.AppendLine();
            sb.AppendLine("Metrics to score:");
            foreach (MetricDefinition metric in request.Metrics)
            {
                string line = $"+ [id: {metric.Id}] {metric.Name}: {metric.Description}".Trim();
                if (metric.MinValue.HasValue)
                {
                    line += $" (minimum value: {metric.MinValue.Value})";
                }
                if (metric.MaxValue.HasValue)
                {
                    line += $" (maximum value: {metric.MaxValue.Value})";
                }
                line += $" (higher is better: {(metric.HigherIsBetter ? "yes" : "no")})";
                sb.AppendLine(line);
            }
            ...
            sb.AppendLine();
            sb.AppendLine("Return ONLY JSON with the schema: {\"scores\":[{\"id\":string,\"score\":int}]}");
